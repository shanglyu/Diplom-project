...
class MealModel
{
    ...
    {
        $sqlState = $this->pdo->prepare("SELECT * FROM meals WHERE meal_id=$meal_id");
        $sqlState->execute();
    ...
    public function addMeal()
    {
        ...
        return $sqlState->execute([$this->meal_name, $this->meal_price, $this->meal_image_url]);
    }
    ...
    public function updateMeal($meal_id)
    {
        $sqlState = $this->pdo->prepare("UPDATE meals SET 
                                            ...
                                            meal_image_url = ? 
                                         WHERE meal_id = ?");
        return $sqlState->execute([$this->meal_name, $this->meal_price, $this->meal_image_url, $meal_id]);
    }

    public function deleteMeal($meal_id)
    {
        $sqlState = $this->pdo->prepare("DELETE FROM meals WHERE meal_id = ?");
        return $sqlState->execute([$meal_id]);
    }
}
